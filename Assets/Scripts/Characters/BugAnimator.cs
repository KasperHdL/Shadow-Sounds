...
public class BugAnimator : MonoBehaviour {
    ...
    public void Update() {
        ...
        for (int i = 0; i < LeftLegs.Length; i++)
            LeftLegs[i].localEulerAngles = new Vector3(0, 0, AnimationLegFactor * LegCurve.Evaluate(t + leftLegOffsets[i]) + legOffsetAngle);
        for (int i = 0; i < RightLegs.Length; i++)
            RightLegs[i].localEulerAngles = new Vector3(0, 0, AnimationLegFactor * -LegCurve.Evaluate(t + rightLegOffsets[i]) - legOffsetAngle);
    }
}
